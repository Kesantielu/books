...
### Description
The script accepts a directory, processes scans previously prepared in Scantailor Advanced, then generates a combined djvu.
### Prepairing
- Process the scans in Scantailor Advanced so the `out` folder need to contains subfolders `foreground` and/or `background`
- The script supports Scantailor color segmentation of scan. This image must not contain a full-color layer in the `background` folder.
